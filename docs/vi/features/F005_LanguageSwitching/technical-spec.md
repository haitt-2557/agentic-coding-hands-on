...
**Linked FR:** FR-002
**Source:** `components/ui/language-switcher.tsx:18-21`
...
**Source:** `design/momorph-hUyaaugye2-node-values.md` § Icon slot (dòng 50-57); code: `components/ui/language-switcher.tsx:35-42`
...
**Transition đặc thù F005:** `open → closed` khi chọn dòng VN/EN có side effect `setLocale(code)` → `localStorage.setItem('saa.locale', code)` (`lib/i18n/locale-provider.tsx:63-66`, kích hoạt từ `components/ui/language-switcher.tsx:63-66`). Hành vi này ĐÃ tồn tại và đã test xanh — liệt kê lại ở đây để mô hình trạng thái của F005 đầy đủ, không phải thay đổi mới.
...
- **SC-006** — Panel/dòng/trigger của `LanguageSwitcher` khớp giá trị thiết kế (FR-020..FR-026): panel chrome, kích thước dòng, icon cờ VN+EN, nền dòng chọn, typography label, cờ trên trigger khi `en` — RED-first: assertion viết trong file mới `e2e/homepage-language-dropdown.spec.ts` (tách riêng khỏi `homepage-dropdown-menus.spec.ts` — file đó không bị đổi trong revision này) trước khi sửa code, chạy RED thật, rồi implement tới GREEN.
...
| 4 | `components/ui/language-switcher.tsx:1-88` | Component chính bị sửa: thêm flag mỗi dòng, nền dòng chọn, trigger đồng bộ + cờ EN |
...
| 9 | `e2e/homepage-dropdown-menus.spec.ts:1-101` | Coverage hành vi hiện có (SC-005, SM-001) phải giữ xanh — file này không bị đổi trong revision |
| 10 | `e2e/homepage-language-dropdown.spec.ts:1-147` | File mới, tách riêng — assertion RED-first cho SC-006 (panel chrome, dòng, icon cờ, nền dòng chọn, trigger) |
...
4. **File:** `components/ui/language-switcher.tsx:1-88` — component chính cần sửa: cấu trúc `options`, render trigger, render mỗi dòng.
