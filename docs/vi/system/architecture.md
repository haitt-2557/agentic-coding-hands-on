...
> **status: implemented** — forward-draft viết ở giai đoạn spec, promote tại implement-start (F015). Được promote vào
...
**Cập nhật (Stage 5 rework) — cơ chế đọc thật, không phải select trực tiếp.** Một vòng rà soát
bảo mật phát hiện: RLS lọc theo HÀNG chứ không theo CỘT, nên grant bảng-rộng đã có sẵn từ F014
(`grant select on profiles to authenticated`) vẫn để lộ `auth_user_id` dù không ai định vậy —
mọi câu lệnh có `where`/`select` đọc cột này trong một session `authenticated` đều hợp lệ. Migration
này thu hẹp lại: `revoke select on profiles from authenticated` rồi cấp lại đúng ba cột
`grant select (id, display_name, department) on profiles to authenticated` — `auth_user_id`
không nằm trong danh sách. Việc tra cứu slug từ `auth.uid()` giờ đi qua một hàm `security definer`
duy nhất, `resolve_viewer_slug(uuid)` (migration `20260825140000_kudos_likes_tables.sql`, gọi từ
`lib/kudos/viewer-identity.ts` qua `supabase.rpc(...)`) — hàm này chạy dưới quyền chủ sở hữu
migration, không phải quyền của `authenticated`, nên đọc được cột đã bị thu hẹp mà không cần trả
lại quyền select trực tiếp cho client.
