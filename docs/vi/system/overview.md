...
`/kudos`, `/profile`, `/admin` là các route placeholder có chủ đích theo brief của dự án, không phải tính năng dang dở. **Cập nhật 2026-08-20**: `/awards` không còn thuộc nhóm này — trang Hệ thống giải thưởng đầy đủ đã thay thế placeholder cũ (`F012_AwardSystemPage`, xem [`docs/vi/features/award-system-page/`](../features/award-system-page/technical-spec.md)): hero thu nhỏ, khối tiêu đề, nav danh mục dính bên trái, 6 khối chi tiết giải thưởng, và nay cũng compose `SiteHeader`/`SiteFooter` như trang chủ.
