...
> **W7-merge note (core-only run):** merged from `core-review-report.md`; no
> `feature-review-batch-*.md` exists in a core pass. Both W7a warnings were resolved
> by the orchestrator before this merge — `route-list.md` Owner F### backfilled from
> `_canonical-fcodes.json` (6/6 cells), and the stale "pending Wave 5" cross-reference
> checkboxes in `screen-list.md` / `user-stories.md` closed against verified 16/16 US
> and 5/5 SCR coverage. `warnings` therefore reads 0 here, 2 in the unmerged W7a report.
