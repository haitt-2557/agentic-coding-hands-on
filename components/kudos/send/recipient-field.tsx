...
// FR-002, US002 — Người nhận autocomplete (frame node I1612:5057;520:9871, 672x56 wrapper /
// 514x56 filled-state input, border #998C5F / border-accent, radius 8px, placeholder "Tìm
// kiếm" per spec B.2 — the frame image's filled value is treated as the filled state of the
// same control, not a second placeholder, per clarifications.md design defect #2).
//
// dom-contract.md:
...
export function RecipientField({ profiles, recipientId, error, onSelect, onBlur }: RecipientFieldProps) {
  ...
  return (
    ...
    <div className="flex w-full max-w-[672px] flex-col gap-2">
      ...
      <div
        className={`relative flex h-14 w-full max-w-[514px] items-center justify-between gap-2 rounded-lg border bg-white px-6 py-4 ${fieldBorderClass(Boolean(error))}`}
      >
        {/* mm:I1612:5057;520:9873 */}
        <input
          id={inputId}
          type="text"
          autoComplete="off"
        ...
        />
        {/* mm:I1612:5057;520:9873;186:1862 */}
